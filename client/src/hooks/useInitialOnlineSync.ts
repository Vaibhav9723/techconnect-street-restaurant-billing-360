  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        /* ================= BILLS ================= */
        // Fetch all remote bills, then merge by id + updatedAt
        const remoteBills = await fetchBills(user.uid);
        const mergedBills = mergeById(
          bills.data,
          remoteBills
        );
        await bills.setData(mergedBills);

        /* ================= UPLOAD LOCAL-ONLY BILLS ================= */
        // Only upload bills that exist locally but NOT in remote
          ...
          for (const bill of bills.data) {
            if (!remoteIds.has(bill.id)) {
              try {
                ...
                console.error("Bill upload failed:", bill.id, e);
              }
            }
          }
        }
